...
return ONLY the predicted disease name not a single letter needed apart from disease name please display disease name only at any cause if u are not able to find the disease name try to give any disease baased on the symptoms what ever match but donot give that based on ysmptoms that type of words just give disease name only please .
"""
# -------------------------
# Helper Functions for Prescription Generation (moved from generate_prescription_logic1.py)
# -------------------------
...
def _pad_to_minimum(plan, min_length=3, is_prescription=True):
    ...
    while len(plan) < min_length:
        ...
        plan.append(base)
    return plan
def _generate_dynamic_prescription(context):
    """Generate dynamic prescription using Gemini agent - real-time, disease-specific."""
    ...
    # Enhanced, dynamic prompt for 100% disease-specific generation (no fixed examples, forces originality)
    prompt = f"""
You are a world-class veterinary pharmacologist and nutritionist. For the disease '{disease_text}' in a {context['animal']} ({context['breed']}, age: {context['age']}, weight: {context['weight']}kg, sex: {context['sex']}, history: {context['medical_history']}), generate a REAL-TIME, EVIDENCE-BASED treatment plan. Base it strictly on current veterinary guidelines (e.g., WSAVA, Merck Vet Manual). Do NOT use generic or placeholder meds—use SPECIFIC, approved drugs/doses for THIS disease only (e.g., for Parvovirus: Amoxicillin/Clavulanate 20mg/kg BID; for Distemper: Anticonvulsants if seizures).

Pet Profile:
- Name: {context['petName']}
- ID: {context['petNo']}
- Symptoms: {', '.join(context['symptoms']) if context['symptoms'] else 'N/A'}

Requirements:
- Output ONLY valid JSON. No intro, no explanations outside JSON.
- At least 4 unique prescription entries: Disease-specific meds (antibiotics for bacterial, antivirals for viral, topicals/steroids for skin/allergies, etc.). If skin disease, include 2+ ointments/creams.
- At least 4 unique diet entries: Tailored nutrition (e.g., low-fat for pancreatitis, hypoallergenic for allergies).
- Dates: Use '{today}' for all.
- Times: IST {morning}, {afternoon}, {evening}, or 'Daily'.
- Dosages: Precise mg/kg (assume 10-20kg pet or 200-500kg large animal; adjust notes for weight).
- Routes: oral, injectable (IM/IV), topical, etc.
- Duration: Specific (e.g., 5-7 days).
- Notes: Include warnings, monitoring, breed-specific adjustments.
- Explanation: 2-3 sentences on rationale, risks, vet urgency. Emphasize: "AI-generated; immediate vet consultation required."
...
{{
  ...
  "prescription_plan": [
    {{
      ...
      "duration": "X days",
      "notes": "Disease-specific rationale; vet administer"
    }},
    // 3+ more, all unique and tailored
  ],
  "diet_plan": [
    {{
      "date": "{today}",
      "feeding_time": "morning",
      "food_type": "SPECIFIC DIET TYPE",
      "quantity": "X g/ml/kg (adjust for weight)",
      "notes": "Rationale for {disease_text}"
    }},
    // 3+ more, tailored (e.g., bland for GI, renal for CKD)
  ],
  "explanation": "Concise, evidence-based summary."
}}

Ensure 100% accuracy: Research internally if needed via tools. Prioritize safety—no experimental treatments.
"""
    try:
        ...
        # If Gemini fails (rare), raise to handle in route - no fallback to avoid fixed plans
        raise RuntimeError(f"Dynamic generation failed: {e}. Ensure API key and model access.")
# -------------------------
...
def generate_prescription():
    data = request.get_json()
    # Fetch data from localStorage via the 'petRecord' object
    pet_record = data.get('petRecord', {})
    animal = pet_record.get('animal', 'Unknown')
    breed = pet_record.get('breed', 'Unknown')
    pet_name = pet_record.get('petName', 'Unnamed')
    pet_no = pet_record.get('petNo', 'PET-' + str(int(pd.Timestamp.now().timestamp())))
    guess = pet_record.get('guess', pet_record.get('disease', 'Unknown')) # Use 'guess' if available, else 'disease'
    if not guess or guess == 'Unknown':
        return jsonify({"error": "No valid disease predicted. Please predict disease first."}), 400
    try:
        # 1) Build context from inputs (disease from prediction is key input)
        context = {
            "disease": guess.strip(),
            "symptoms": pet_record.get('symptoms', []),
            "breed": breed,
            "age": data.get("age", ""),
        ...
        # 2) Generate dynamic, real-time prescription using Gemini (replaces old logic import)
        result = _generate_dynamic_prescription(context)
        app.logger.info("✅ Dynamic Gemini generation succeeded for disease: %s", guess)
        # 3) Add pet details to result for PDF rendering
        result['petName'] = pet_name
        result['petNo'] = pet_no
        # 4) Generate PDF
        try:
            result['pdf_url'] = generate_pdf(result, pet_no)
        except Exception:
            app.logger.exception("Failed to generate PDF — continuing without pdf_url")
            result['pdf_url'] = ''
        # 5) Return final JSON
        return jsonify({
            "animal": animal,
