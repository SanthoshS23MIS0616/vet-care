...
Analyze the uploaded animal image and return only the predicted disease name and its probability (out of 100) in this format: Disease: [Name] Probability: [XX]
"""
...
# -------------------------
# Prescription Logic using Gemini (defined here for robustness)
# -------------------------
def generate_prescription_logic(data):
    animal = data.get('animal', 'Dog')
    breed = data.get('breed', 'Unknown')
    pet_name = data.get('pet_name', 'Pet')
    disease = data.get('guess', data.get('disease', 'Unknown Disease'))  # Use clean 'guess' from prediction

    current_date = datetime.now().strftime('%Y-%m-%d')

    prompt = f"""
You are a veterinary expert. Generate a detailed, accurate prescription and recovery plan for a {animal} {breed} named {pet_name} diagnosed with {disease}.

For skin-related diseases, include at least 2 ointments/topicals with specific names, dosages, and application notes.
...
{{
    "prescription_plan": [
        {{
            "date": "{current_date}",
            "time": "09:00",
            "medicine": "[Medicine Name 1]",
            "dosage": "[e.g., 10mg/kg]",
            "route": "[e.g., Oral]",
            "duration": "[e.g., 7 days]",
            "notes": "[Specific notes]"
        }},
        // Add at least 3-5 total rows, including ointments if skin disease
        // ...
    ],
    "diet_plan": [
        {{
            ...
            "food_type": "[Food 1 e.g., Boiled chicken and rice]",
            "quantity": "[e.g., 1 cup]",
            "notes": "[Notes for recovery]"
        }},
        {{
            "date": "{current_date}",
            "feeding_time": "evening",
            "food_type": "[Food 2 e.g., Yogurt with probiotics]",
            "quantity": "[e.g., 1/2 cup]",
            "notes": "[Notes for recovery]"
        }}
        // Add at least 2 rows
    ],
    "explanation": "[2-4 lines of owner-friendly advice, e.g., Monitor symptoms, consult vet if worsens...]"
}}

Ensure medicines are real/vet-appropriate for {animal}/{breed}/{disease}, dosages safe/accurate.
"""

    try:
        response = medical_agent.run(prompt)
        result = json.loads(response.content.strip())
        result['disease'] = disease  # Include for template
        return result
    except json.JSONDecodeError:
        # Fallback stub if JSON fails
        return {
            "error": "Failed to generate prescription. Please consult a vet.",
            "prescription_plan": [],
            "diet_plan": [],
        ...
        raise RuntimeError(f"Prescription generation error: {e}")
...
def predict_skin_gemini():
    ...
    # Parse to extract clean disease name (e.g., "Ringworm" or "Lumpy Skin Disease")
    # Assume format: "Disease: [Name] Probability: [XX]"
    predicted_disease = "Unknown Disease"
    import re
    match = re.search(r'Disease:\s*([^\s][^P]+?)(?:\s*Probability|$)', raw, re.IGNORECASE)
    if match:
        predicted_disease = match.group(1).strip()

    # For backward compat, keep 'result' as full, but add 'predicted_disease' for clean use
    result = next((line for line in raw.splitlines() if line.strip()), raw)
    return jsonify({"result": result, "predicted_disease": predicted_disease, "raw": raw})
