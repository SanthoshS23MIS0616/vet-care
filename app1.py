...
def hwl_clean(height):
    ...
    nums = [float(p) for p in parts if is_number(p)]
...
mean_life = (df["life_c"].sum() - df["life_c"].max()) / (len(df)-1)
...
df_euc = df[euclidean_cols].copy()
df_euc.fillna(df_euc.mean(), inplace=True)
normalized_df_euc = (df_euc - df_euc.min())/(df_euc.max()-df_euc.min())

def get_names(profile):
    dists = [np.linalg.norm(profile - normalized_df_euc.iloc[i])
             for i in range(len(normalized_df_euc))]
    idxs = sorted(range(len(dists)), key=lambda i: dists[i])[:5]
    names = df['dog'].values[idxs]
    similarities = 1 - (sorted(dists)[:5]/max(dists))
...
def recommend_breed():
    ...
    names, similarities = get_names(profile)
    base = 'https://www.akc.org/dog-breeds/'
    result = [
        {'name':n, 'similarity':s, 'url':base + n.lower().replace(' ', '-')}
...
def predict_disease():
    ...
    vec = np.array(symptoms).reshape(1, -1)
    pred = joblib_model.predict(vec)
    idx  = np.argmax(pred)
    mapping = {
        0: ('Tick fever', 'https://www.thevetexpert.com/...'),
        1: ('Distemper', 'https://www.akc.org/...'),
        2: ('Parvovirus', 'https://www.akc.org/...'),
        3: ('Hepatitis', 'https://vcahospitals.com/...'),
        4: ('Tetanus', 'https://vcahospitals.com/...'),
        5: ('Chronic kidney Disease', 'https://www.petmd.com/...'),
        6: ('Diabetes', 'https://www.petmd.com/...'),
        7: ('Gastrointestinal Disease', 'https://www.petmd.com/...'),
        8: ('Allergies', 'https://www.petmd.com/...'),
        9: ('Gingivitis', 'https://www.petmd.com/...'),
        10:('Cancers', 'https://www.petmd.com/...'),
        11:('Skin Rashes', 'https://www.petmd.com/...')
    }
    disease, url = mapping.get(idx, ("Unknown Disease", "#"))
    return {"disease": disease, "url": url}

# -------------------------
# Gemini-based Skin Analysis
# -------------------------
