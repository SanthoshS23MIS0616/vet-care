...
return ONLY the predicted disease name not a single letter needed apart from disease name.
"""
...
def generate_prescription():
    ...
    # Fetch data from localStorage via the 'guess' variable and other fields
    ...
    guess = pet_record.get('disease', data.get('disease', 'Unknown'))

    try:
        # 1) Run your Gemini logic with current India time and guess variable
        result = generate_prescription_logic({
            "disease": guess,
        ...
        # 3) Return final JSON with animal, breed, petName, petNo in top right corner data
        return jsonify({
            "animal": animal,
            "breed": breed,
            "petName": pet_name,
